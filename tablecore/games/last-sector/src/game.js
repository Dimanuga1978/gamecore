...
function createGame(options = {}) {
  return {
    ...
    applyAction(state, action, context = {}) {
      // Real bug found via a real external audit, confirmed directly:
      // packages/core/src/runAction.js's own canonical dispatcher calls
      // `game.applyAction(workingState, safeAction, context)` -- THREE
      // arguments, with `context` (carrying `rng`/`seed`) as the third --
      // but this function used to only declare TWO parameters and
      // hardcode an empty `{}` when forwarding to applyActionInPlace,
      // silently dropping whatever real context it was actually given.
      // Since applyActionInPlace unconditionally rejects with
      // RNG_CONTEXT_REQUIRED when context.rng is missing, this meant
      // calling applyAction() directly (part of this pack's own declared
      // public GameDefinition contract, per packages/game-api's own doc
      // comment) failed for EVERY legal action, unconditionally --
      // confirmed directly by calling it: `lastSector.applyAction(state,
      // {type:'END_TURN',actor:'A'})` returned RNG_CONTEXT_REQUIRED even
      // for a freshly-created, valid match. Fixed by simply accepting
      // and forwarding the context this function was always actually
      // being called with, matching the contract runAction.js already
      // establishes -- not a design choice between two options, just a
      // dropped parameter.
      return this.applyActionInPlace(clone(state), action, context);
    },
